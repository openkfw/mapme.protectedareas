...
dir.create("results/treeloss", showWarnings = FALSE)

start_time = Sys.time()

for(i in 1:length(aoi3)){
  filename = file.path("results/treeloss", paste0("honeycomb_", i, ".rds"))
...
rds_files = list.files("results/treeloss/", pattern = ".rds$", full.names = TRUE)
data = lapply(rds_files, function(f) readRDS(f))
data = st_as_sf(purrr::map_dfr(data, rbind))
saveRDS(data, "honeycomb_5sqkm_treeloss.rds")
