...
# Mangrove Gain and Loss (supported wdpa) --------------------------------------

mangrove_supported <- function(y) {
  
  b <- st_read("/home/ombhandari/shared/datalake/mapme.protectedareas/output/polygon/wdpa_kfw/wdpa_kfw_spatial_latinamerica_2021-02-01_supportedPAs.gpkg")
  m <- st_read(paste0("/home/ombhandari/shared/datalake/mapme.protectedareas/input/global_mangrove_watch/gmw-v2-",y,"-valid.gpkg"))
  b <- st_transform(b, st_crs(m))
  m_subset <- st_intersection(m, b)
  m_sub <- st_transform(m_subset, st_crs(area_proj(b)))
  m_sub$area_sqm <- st_area(m_sub)
  return(m_sub)
}

#mangrove_supported("1996")
#mangrove_supported("2007")


# Mangrove Gain and Loss (all wdpa) --------------------------------------------

mangrove_all <- function(y) {
  
  b <- st_read("/home/ombhandari/shared/datalake/mapme.protectedareas/output/polygon/wdpa_kfw/wdpa_kfw_spatial_latinamerica_2021-02-01_allPAs.gpkg")
  m <- st_read(paste0("/home/ombhandari/shared/datalake/mapme.protectedareas/input/global_mangrove_watch/gmw-v2-",y,"-valid.gpkg"))
  b <- st_transform(b, st_crs(m))
  m_subset <- st_intersection(m, b)
  m_sub <- st_transform(m_subset, st_crs(area_proj(b)))
