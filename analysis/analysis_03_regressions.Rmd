...
est_table <- matrix(NA, nrow = 1, ncol = length(all_years)+1)
colnames(est_table) <- c("name", all_years)
est_table[1,1] <- "Treatment effect estimate"

est_table_m12 <- est_table
est_table_m22 <- est_table
est_table_m32 <- est_table
...
for (t in all_years) {
 ...
 # create time varying treatment variable
  ...
  panel.df$WDPAID_cluster <- as.numeric(paste0(panel.df$NAME_0_num, panel.df$WDPAID_cluster)) 
  ...
  #m12
  est_table_m12[1,paste0(t)] <- get(paste0("m12_",t))$coeftable[1]
  
  #m22
  est_table_m22[1,paste0(t)] <- get(paste0("m22_",t))$coeftable[1]
  
  #m32
  est_table_m32[1,paste0(t)] <- get(paste0("m32_",t))$coeftable[1]
...
m_list3 <- list(m32_2004, m32_2005, m32_2006, m32_2007, m32_2008, m32_2009, m32_2010, m32_2011, m32_2012, m32_2013, m32_2014, m32_2015, m32_2016, m32_2019, m32_2019)
...
```{r save obs and est table}

# Observations tables
write_rds(obs_table_m12,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/obs_table_m12.rds")

write_rds(obs_table_m22,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/obs_table_m22.rds")

write_rds(obs_table_m32,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/obs_table_m32.rds")

# Estimate tables
write_rds(est_table_m12,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/est_table_m12.rds")

write_rds(est_table_m22,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/est_table_m22.rds")

write_rds(est_table_m32,
          "/datadrive/datalake/mapme.protectedareas/output/tabular/regression_output/est_table_m32.rds")
