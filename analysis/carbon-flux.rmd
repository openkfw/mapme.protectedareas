...
knitr::opts_knit$set(root.dir = "/home/ombhandari/shared/Om/mapme.protectedareas/")
library(rmarkdown)
library(terra)
library(rgdal)
library(tmap)
...
To calculate zonal statistics for net forest carbon flux that changed between 2001 to 2019, enlisted is the required processing routine:
  ...
  - Fetch country level wdpa data from library `wdpar` and clean the data
...
- you can call the function `get_net_carbon_flux` by passing (lat, lon) arguments for eg. ("10S", "050W") or ("10N", "020E")
...
- or simply visit the [GFW Dataset Portal](https://data.globalforestwatch.org/datasets/net-forest-carbon-flux/data?geometry=163.031%2C-61.608%2C31.547%2C83.830) to verify chosen grid coordinates
...
Since there are more than 3000 enlisted protected areas in Brazil, we want to have a look at the polygon data of:
  - Reserva Biologica Do Rio Trombetas - wdpaid 43 
	- Reserva Extrativista Rio Cajari - wdpaid 31776
