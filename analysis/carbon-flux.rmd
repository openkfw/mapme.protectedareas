...
library("wdpar")
library("dplyr")
library("mapview") # used for interactive maps in this website; not essential for analysis
...
knitr::include_graphics("docs/assets/workflows/carbon_flux_zonal_stats.jpg")
```
...
At first you should link to the source functions to go through this routine. The script `carbon-flux.R` contains the function to download the raster file of the desired grid.
...
# load raster for 00N, 060W
myRaster_ar <- rast("/home/rstudio/shared/datalake/mapme.protectedareas_net-forest-carbon-flux-00N-060W_raw.tif")
```
...
Note: For this routine, we are using `myRaster`, the one from API.
...
myCrop <- terra::crop(myRaster, myExtent_v)
