...
### Get centroid
The function `wdpalevel_centroid` takes wdpaid as argument and returns the centroid of the WDPA poylgon (x,y) in EPSG 4326 (Lat Long WGS84). For polygon PAs centroids are calculated with the function ST_PointOnSurface, which returns a point guaranteed to lie on the surface.
...
### Get water surface area 
4. The function `get_water_stats` takes wdpaid as argument. 
...
### Get landcover change from ESA
The function `get_landcover_esa` takes wdpaid as argument and returns absolute cover of ESA LC CCI classes (aggregated by level 1: 4 classes) which changed within first and last epoch i.e. 1995 & 2015 for a given PA
...
- Pass the year for the statistics end
- Pass aggregation level
*TODO*: Explain shortly what the aggregation levels are 
...
df_landcover_esa<-
  get_landcover_esa(32671,year = 2000,agg=0)
```
...
### Get laddcover ESA
TODO: HOW IS THIS DIFFERENT FROM THE ONE ABOVE? I guess the one above with change should be probabla `get_lcc_esa` right? So you would also need to adapt the description. 
