...
for (i in T_year) {
  ...
  mp_cem <- 
    read_csv(paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/t3/cem_t3_matched_panel_", i, ".csv")) %>% 
  ...
  mp_psm <- 
    read_csv(paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/PSM/t3/psm_t3_matched_panel_", i, ".csv")) %>% 
    mutate(fc_area_weighted = fc_area * weights)
  ...
  avg_plot_cem <- 
    ...
    scale_x_continuous(breaks = seq(i-3, 2020, by = 2))
