...
# get packages
lop <- c("dplyr", "plm", "stargazer", "tidyverse", "MatchIt", "glm", "optmatch", "cobalt", "gridExtra", "sf")
...
# drop BMZ Numbers before with first_year being 2000 as there are no loss_tn data
bmz_list <- bmz_list[-which(bmz_list=="199865056")]
...
for (bmz in bmz_list) {
  ...
  # Load data
  df <- read_rds(paste0("/datadrive/datalake/mapme.protectedareas/output/matching/matching_frames/projects/matching_frame_",bmz,".rds")) %>% merge(., keys_assetid_wdpa, by=".assetid", all.x=T)
...
first_year_num <- length(unique(df$first_year[!is.na(df$first_year)]))
    if (first_year_num ==1){ #if only one year in first year (without counting NA)
      ...
      year2 <- list_year$first_year[which(list_year$num_asset==max(list_year$num_asset))]
      df$first_year <- year2
      year <- unique(df$first_year_bmz)
...
#  tryCatch({
    ...
    # Save matchit object
    write_rds(m_out1, paste0("/datadrive/datalake/mapme.protectedareas/output/matching/matchit_objects/PSM/tn_projects_test/m_psm_tn_", bmz, ".rds"))
    ...
    write_rds(m_data1, 
            paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/PSM/tn_projects_test/psm_tn_matched_data_", bmz, ".rds"))
...
first_year_num <- length(unique(df$first_year[!is.na(df$first_year)]))
  ...
  # Save matchit object
  write_rds(m_out2, paste0("/datadrive/datalake/mapme.protectedareas/output/matching/matchit_objects/CEM/tn_projects_test/m_cem_tn_", bmz, ".rds"))
  ...
  write_rds(m_data2, 
            paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/tn_projects_test/cem_tn_matched_data_", bmz, ".rds"))
