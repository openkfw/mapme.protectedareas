...
total_treated_area <- obs_treated * 500
...
# add kfw finance per ha
disbursement_per_ha <- disbursement/total_treated_area
...
# rbind tables
effects_table <- as.data.frame(rbind(
                                     # loss_effect,
                                     # emissions_effect,
                                     # obs_treated,
                                     total_treated_area,
...
effects_table <- subset(effects_table, select = c(-`2010`, -`2013`, -`2014`, -`2016`, -`2017`, -`2019`))


total_1 <- sum(effects_table[1,])
total_2 <- sum(effects_table[2,])
total_3 <- sum(effects_table[3,])
total_4 <- sum(effects_table[4,])
total_5 <- total_4/total_3

# total
totals <- c(total_1, total_2, total_3, total_4, total_5)

effects_table <- cbind(effects_table, totals)

rownames(effects_table) <- c(
                             # "Forest cover loss avoided per cell (ha)", "CO2 emissions avoided per cell (tons)", "Number of supported PA cells", 
                             "Total supported protected area (ha)", 
                             "Total forest cover loss avoided (ha)", 
                             "Total CO2 emissions avoided (tons)", 
                             "KfW finance (EUR)", 
                             # "Finance per ha (EUR)",
                             # "Emissions avoided per EUR invested", 
                             "Euro per avoided ton of CO2 emissions")
