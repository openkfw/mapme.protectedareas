...
bmz_list <- bmz_list[-which(bmz_list=="199566829")] # skip entries that cause issues because of perfect collinearity of treatment variable
# load each list entry and create unique panel id across all matching frames
big.list.of.data.frames <- lapply(bmz_list, function(x){
...
big.data.frame <- do.call(rbind,big.list.of.data.frames)
 ...
 panel.df <- big.data.frame %>% 
  ...
  # save big file
  saveRDS(panel.df, file="/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/cem_matched_data_stacked_projects.rds")
