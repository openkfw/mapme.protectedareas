---
title: "Centra America Portfolio -- BMZ Project Overview"
author: "Andreas Petutschnig, Leonie Gallé"
date: "2023-11-22"
...
bmz_projects <- unique(dat_portfolio$`BMZ Number`)
n_bmz_projects <- length(bmz_projects)
col <- brewer.pal(n = n_bmz_projects, name = "Set3")
group_names <- paste0("BMZ Project ", bmz_projects)
...
for(i in seq_len(n_bmz_projects)) {
  polys <- dat_portfolio %>%
    filter(`BMZ Number` == bmz_projects[i])
  poly_color <- col [i]
...
duplicate_table <- dat_portfolio %>%
  st_drop_geometry() %>%
  filter(`WDPA ID` %in% wdpa_duplicates) %>%
  select(`WDPA ID`, `BMZ Number`, `Short Name`) %>%
  unique() %>%
  arrange(., `WDPA ID`)
