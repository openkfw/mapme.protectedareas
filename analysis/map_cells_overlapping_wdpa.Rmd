...
# get packages
lop <- c("tidyverse", "sf", "leaflet", "leaflet.extras", "leaflet.extras2", "ggsci", "scales", "htmltools", "RColorBrewer", "raster", "rgdal", "stars", "kableExtra")
...
# Create a table 
year_table <- data.frame(table((check_data_merged %>% 
              # filter(first_year == year) %>%
              distinct(key, .keep_all=TRUE) %>% 
              group_by(first_year,.assetid) %>% 
              count() %>% 
              filter(n>1))$first_year),
              obs_treated)
colnames(year_table) <- c("Year", "Cells with multiple WDPA IDs", "Total number of treated cells")
year_table %>% 
  kbl() %>% 
