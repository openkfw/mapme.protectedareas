...
## Crate map
# wdpa_kfw_centroid<-st_transform(st_centroid(wdpa_kfw_centroid),crs = 4326)
my_map <-
  leaflet() %>%
  ...
  addTiles("https://tiles.globalforestwatch.org/umd_tree_cover_loss/v1.8/dynamic/{z}/{x}/{y}.png",group="Forest Cover Loss (2001-2020)",attribution = "Forest Cover Loss: Hansen,et al. 2013. “High-Resolution Global Maps of 21st-Century Forest Cover Change.” Science 342 (15 November).")%>%
  # add own data
  ...
  # addCircles(data = wdpa_kfw_centroid, opacity = 0.9,color = ~pal(MARINE), group = "Type",label = ~htmlEscape(NAME))%>%
  # addCircles(data = wdpa_kfw_centroid, opacity = 0.9,color = ~pal2(bmz_n_1),group = "Project-Nr.")%>%
