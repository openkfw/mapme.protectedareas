...
my_map <-
  ...
  addPolygons(data = selva_maya_buffer, opacity = 0.1, fillOpacity = 0.1, color = "#7d0f02",
              group = "Buffer Regions", weight = 0.2) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("PAs Selva Maya","Buffer Regions", "PAs (Others)", "Labels (PA Names)", "Sum of Forest Cover Loss (2001-2020)","Forest Cover Loss (2001-2020)", "Regional Primary Forests (2001)"),
...
my_map <-
  ...
  addLayersControl(
    baseGroups = c("CartoDB","OpenStreetMap","Satellite","Topography","Nightlights"), #"Toner",,"Regional Primary Forests (2001)"
    overlayGroups = c("PAs Selva Maya","Buffer Regions", "PAs (Others)", "Labels (PA Names)", "Active Fires Count (2000-2021)","Forest Cover Loss (2001-2020)", "Regional Primary Forests (2001)"),
    options = layersControlOptions(collapsed = FALSE)) %>%
  # ommit certain layers
  hideGroup(group = c("PAs (Others)","Buffer Regions","Forest Cover Loss (2001-2020)", "Regional Primary Forests (2001)","Labels (PA Names)")) %>%
  fitBounds(bbox [1], bbox [2], bbox [3], bbox [4])
...
p <- ggplot(fires, aes(x = month, y = monthly_fires, group = year)) +
  stat_summary(fun.y = mean, geom = "line", color = "#ababab") +
  geom_point(alpha = 0.1) +
  theme_classic() +
  scale_x_continuous(breaks = 1:12) +
  xlab("Month") +
  ylab("Fires per Month") +
  ggtitle("Monthly Fire Events and Mean Monthly Fire Events") +
  facet_wrap(vars(extent), ncol = 1, nrow = 2, scales = "free")

ggplotly(p)
```

TODO: Text about correlation between fire counts and intensity

```{r fire-correlation, message = FALSE, warning = FALSE,fig.width=10, fig.height=10, echo=FALSE, eval=TRUE}
cor_sp <- cor(fires$monthly_fires, fires$monthly_sum_frp_gw, method = "spearman")
...
```{r plots_landcover, message = FALSE, warning = FALSE, eval = FALSE}
# esa_landcover_fixed <- esa_landcover %>%
#   filter(date != max(date)) %>%
#   mutate(landcover_area_ha = landcover_area_ha * 10) %>%
#   bind_rows(esa_landcover %>% filter(date == max(date)))

esa_landcover_dummy <- esa_landcover %>%
#esa_landcover_dummy <- esa_landcover_fixed %>%
  expand(landcover_class, date, extent) %>%
  mutate(landcover_area_ha = 0)

cols = c("#66C2A5", "#FC8D62", "#8DA0CB", "#E78AC3", "#A6D854", "#FFD92F", "#E5C494", "#B3B3B3")
lc_plot <- esa_landcover %>%
#lc_plot <- esa_landcover_fixed %>%
  bind_rows(esa_landcover_dummy) %>%
  mutate (date = year(as.Date(date))) %>%
  group_by(landcover_class, date, extent) %>%
  summarize(landcover_area_ha = sum(landcover_area_ha), .groups = "rowwise") %>%
  group_by(date == max(date), extent) %>%
#  group_by(extent) %>%
  do(p = plot_ly(., x = ~as.factor(date), y = ~landcover_area_ha, type = "bar",
...
subplot(style(lc_plot$p[[1]] %>%
                layout(annotations = list(text = "Buffered", x = 0.4, y = 1.07,
                                          showarrow = FALSE, xref = "paper", yref = "paper")),
                showlegend = TRUE),
        style(lc_plot$p[[2]] %>%
                layout(annotations = list(text = "Original", x = 0.6, y = 1.07,
                                          showarrow = FALSE, xref = "paper", yref = "paper")),
                showlegend = FALSE),
        style(lc_plot$p[[3]], showlegend = FALSE),
        style(lc_plot$p[[4]], showlegend = FALSE),
        nrows = 2, shareX = FALSE, shareY = FALSE)
```

```{r plots_landcover_2019, message = FALSE, warning = FALSE, eval = TRUE}
esa_landcover_dummy <- esa_landcover %>%
...
#  group_by(landcover_class, extent, wdpa_id) %>%
  group_by(extent) %>%
...
```{r plots_population, message = FALSE, warning = FALSE, eval = TRUE}
p <- ggplot(population, aes(x = date, y = population, color = as.factor(wdpa_id))) +
  geom_point() +
  facet_wrap(vars(extent), ncol = 1, nrow = 2)
...
population$perc_diff_rel <- c(NA, (100 * diff(test$population, lag = 1))) / test$pop_2k
population$perc_diff_rel [population$date == min(population$date)] <- 0

ggplot(population, aes(x = date, y = perc_diff_rel, group = as.factor(wdpa_id),
                       color = as.factor(wdpa_id)
                       )) +
  geom_line() +
  facet_wrap(vars(extent), ncol = 2, nrow = 1)
```
//TODO add population graphs (yearly data for 2000 - 2020)
