...
custom_cem <- function(year, cutoffs_travel_time, cutoffs_loss_t3, cutoffs_elevation, cutoffs_tri, cutoffs_soil) {
  ...
  # Load data
  df <- 
    read_rds(paste0("/datadrive/datalake/mapme.protectedareas/output/matching/matching_frames/matching_frame_",year,".rds")) %>% 
    select(.assetid,
           treatment,
           starts_with("affected_hurricane"),
           starts_with("not_affected_hurricane_"),
           starts_with("loss_tn"),
  ...
  # Save matchit object
  write_rds(m_out2, paste0("/datadrive/datalake/mapme.protectedareas/output/matching/matchit_objects/CEM/TEST_m_cem_tn_", year, ".rds"))
  ...
  ## Export data
  # write_csv(m_data2,
  #           paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/cem_matched_data_", year, ".csv"))
  write_rds(m_data2,
            paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/cem_matched_data_tn_", year, ".rds"))
...
cutoffs_loss_tn <- c(-1,0, 1, 10, 100, 200, 300, 400, 500, 600)
cutoffs_elevation <- c(0,500,1500,6138.56)
cutoffs_tri <- c(0,5,10, 50, 100, 182.1068)
...
year <- 2004
# Call function
custom_cem(year = year, cutoffs_travel_time = cutoffs_travel_time, cutoffs_loss_t3 = cutoffs_loss_tn, cutoffs_elevation = cutoffs_elevation, cutoffs_tri = cutoffs_tri, cutoffs_soil = cutoffs_soil)
```
...
# Assign values to input parameters
cutoffs_travel_time <- c(0,120,300, 400, 500, 600, 800, 1000, 1500, 2000, 3000, 12015.71)
cutoffs_loss_t3 <- c(-1,0, 1, 10, 100, 200, 300, 400, 500, 600)
cutoffs_elevation <- c(0,500,1500,6138.56)
cutoffs_tri <- c(0,5,10, 50, 100, 182.1068)
cutoffs_soil <- c(2.440179, 6.091119,  9.742059, 13.392999, 17.043938, 20.694878, 24.345818, 27.996758, 31.647698, 35.298637, 38.949577, 42.600517, 46.251457, 49.902397, 53.553337, 57.204276, 60.855216, 64.506156)
year <- 2005
...
# Assign values to input parameters
cutoffs_travel_time <- c(0,10, 20, 30, 60, 90, 120, 150, 180, 240, 300, 400, 500, 600, 800, 1000, 1500, 2000, 3000, 6000, 12015.71)
cutoffs_loss_t3 <- c(-1,0, 1, 10, 100, 200, 300, 400, 500, 600)
cutoffs_elevation <- c(0, 50, 100, 200, 300, 400, 500, 1000, 1500,6138.56)
cutoffs_tri <- c(0,5,10, 20, 30, 40, 50, 100, 182.1068)
cutoffs_soil <- c(0, 5,14,40,100)
year <- 2010
# Call function
custom_cem(year = year, cutoffs_travel_time = cutoffs_travel_time, cutoffs_loss_t3 = cutoffs_loss_t3,cutoffs_elevation = cutoffs_elevation, cutoffs_tri = cutoffs_tri, cutoffs_soil = cutoffs_soil)
```
