...
bolivia_suggested_PAs %>%
  ...
  DT::datatable(.,
                ...
                options = list(
                  ...
                  scrollX = TRUE,
                  scrollY = "600px",
                  pageLength = nrow(bolivia_suggested_PAs)
                ))
...
my_map <-
  ...
  addPolygons(data = wdpa_kfw_treated,opacity = 0.9,color = "#2b8cbe", group = "PAs KfW-supported",label = ~htmlEscape(ORIG_NAME),weight = 1)%>%
  addPolygons(data = wdpa_nonSupportPA,opacity = 0.9,color = "darkgrey", group = "PAs (Others)",label = ~htmlEscape(ORIG_NAME),weight = 2)%>%
  addPolygons(data = bolivia_suggested_PAs,opacity = 1, fillOpacity = 0.1, color = "#7d0f02", group = "PAs Suggested",label = ~htmlEscape(ORIG_NAME),weight = 2)%>%
...
# load data
bol_annual_fires <- 
  read.csv("data/bolivia_annual_fires_2000_2021.csv")

# remove duplicates
bol_annual_fires<-bol_annual_fires[!duplicated(bol_annual_fires),]
...
# add names
fire_lines_aux_data <- st_drop_geometry(wdpa_allPAs) %>%
  select(c(WDPAID, bmz_n_1, ORIG_NAME))
...
# merge names
fires_long <-
  merge (fires_long, fire_lines_aux_data, by = "wdpa_id", all.x = TRUE)

# merge missing data
fires_long <- 
  merge(fires_long, bol_annual_fires, all = TRUE)
