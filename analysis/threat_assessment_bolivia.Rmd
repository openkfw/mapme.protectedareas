...
Below you can find an overview of the analyzed areas. Data was downloaded from the WDPA. Some of the analyzed areas appear to be overlapping and some also show two entries in the WDPA. The following data gives some insights into governance variables as provided by the WDPA. You can also see if some of these areas where already supported by a KfW project. 

```{r table, echo=FALSE, warning=FALSE,  fig.width=10}
...
lollipop_data$display_name <- 
        paste0(lollipop_data$NAME, " (WDPA-ID: ", lollipop_data$WDPAID, ")")
...
gfw_lossstats_2 <-gfw_lossstats_2 %>% 
  filter(WDPA_PID %in% bol_wdpa_ids)
...
# # merge names
# gfw_lossstats_2_long <-merge (gfw_lossstats_2_long, 
#                               gfw_lossstats_2, by = "wdpa_id", all.x = TRUE)
# # merge missing data

gfw_lossstats_2 <- merge(gfw_lossstats_2,
                              fire_lines_aux_data, all = TRUE)

# create unique names
gfw_lossstats_2$name_unique<-paste(gfw_lossstats_2$NAME," (WDPA-ID: ",gfw_lossstats_2$wdpa_id,")",sep="")
...
### Absolute Trend
The following "Lollipop Plots" are be used to quickly visualize the total number of fires and sort protected areas from low to high regarding the total number of fire occurences. The figure furthermor shows if any of these PAs has been or is currently supported by a project from KfW. You can get detailed statistics by hovering over the lollipops with your mouse. 
...
### Yearly Firecounts
Heatmaps are used to assess the time-trend of one or several PAs. They show the variation in the total number of fires which can be attributed to human activities or natural factors such as El Niño and La Niña (For more information see [here](https://www.noaa.gov/education/resource-collections/weather-atmosphere/el-nino)). Especially in 2010 where almost all PAs expirienced increased fire-rates, the causes might be related to a strong El-Nino from 2009-2010. 

Green color suggest a low number of fire whereas red colors indicate a larger number You can get more detailed information by hovering with the mouse over the the individual cells.  
...
bol_annual_fires <-
  bol_annual_fires %>%
  group_by(wdpa_id, year) %>%
  summarise(active_fire_counts = sum(active_fire_counts))
...
# add missing values
yrs <- sort(unique(bol_annual_fires$year))
wdpa_ids <- unique(bol_annual_fires$wdpa_id)

fires_long <- data.frame(
  wdpa_id = rep(wdpa_ids, each = length(yrs)),
  year = rep(yrs, times = length(wdpa_ids))
)

# merge names
fires_long <-merge (fires_long, fire_lines_aux_data, by = "wdpa_id", all.x = TRUE)
# merge missing data
fires_long <- merge(fires_long, bol_annual_fires, all = TRUE)
# create unique names
fires_long$name_unique<-paste(fires_long$NAME," (WDPA-ID: ",fires_long$wdpa_id,")",sep="")
# merge with maximum data
# merge total forest loss data for sorting
fires_long_2 <-
  lollipop_data %>%
  select(WDPAID, active_fire_counts) %>%
  rename(active_fire_counts_sum=active_fire_counts) %>% 
  merge(fires_long,
        .,
        by.x = "wdpa_id",
        by.y = "WDPAID",
...
fires_heatmap <-
  ggplot(fires_long_2,
         aes(
           x = year,
           y = reorder(name_unique,-active_fire_counts_sum),
           fill = active_fire_counts,
           text = paste(
             "Area Name: ",
             NAME,
             "\nCount of Active Fires ",
             round(active_fire_counts_sum, digits = 0),
             "\nWDPA - ID: ",
             wdpa_id
           )
         )) +
...
* Combine count, intensity and affected area in a combined estimator
