...
my_map <-
  ...
  addCircleMarkers(data=wdpa_allPAs_lossdata,
                   ...
                   radius = ~ifelse(fires_categorized == "> 10000", 20, 
                                    ifelse(fires_categorized == "5,001-10,000", 12,
                                           ifelse(fires_categorized == "2501-5,000", 8,
                                                  ifelse(fires_categorized == "501-2,500", 4,
                                                         ifelse(fires_categorized == "0-500", 2, 0
                                                         )
                                                  )
                                           )
                                    )),
  ...
  addLayersControl(
    ...
    overlayGroups = c("PAs Suggested", "PAs KfW-supported","PAs (Others)", "Active Fires (2000-2021)", "Relative Forest Loss (2001-2020)","Forest Cover Loss (2001-2020)", "Regional Primary Forests (2001)"),
    options = layersControlOptions(collapsed = FALSE)) %>%
  # ommit certain layers
  hideGroup(group = c("PAs KfW-supported", "Relative Forest Loss (2001-2020)", "PA Area Size","Forest Cover Loss (2001-2020)", "Regional Primary Forests (2001)"))
...
```{r lollipop_plot, message = FALSE, warning = FALSE,fig.width=10, echo=FALSE, eval = FALSE}
...
loss_data_summarycats_absolute$`% of Suggested PAs` <-
  wdpa_allPAs_lossdata %>%
  filter(WDPAID %in% bol_wdpa_ids) %>%
  {
    round(table(.$absolute_loss_categorized) / nrow(.) * 100, digits = 2)
  ...
  as.data.frame() %>%
  rename("Absolute Forst Loss (ha)" = Var1,
         "% of Suggested PAs" = Freq) %>%
  pull("% of Suggested PAs")

kable(loss_data_summarycats_absolute)

# create relative loss tables
...
loss_data_summarycats_relative$`% of Suggested PAs` <-
  wdpa_allPAs_lossdata %>%
  filter(WDPAID %in% bol_wdpa_ids) %>%
  {
    round(table(.$relative_loss_categorized) / nrow(.) * 100, digits = 2)
...
```{r jitter_plot, message = FALSE, warning = FALSE,fig.width=10, echo=FALSE, eval=FALSE}
...
```{r line_plots, message = FALSE, warning = FALSE,fig.width=10, echo=FALSE, eval=FALSE}
...
lossdata_absolute_lineplot <-
  ...
  labs(x="",title="Absolute Forest Loss in PAs in Bolivia")+
  scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),name="Forest Loss in ha")+
...
lossdata_relative_lineplot <-
  ...
  geom_line(aes(x=year,
                ...
                text = paste(
                  ...
                  #"\nRelative forest loss: ", round(relative_loss,digits = 2)*100, " %",
                  "\nBMZ-Number: ", bmz_n_1)) ,alpha=0.4)+
  labs(x="",title="Relative Forest Loss in PAs in Bolivia")+
...
ggplotly(fires_absolute_lineplot,tooltip="text")
```
