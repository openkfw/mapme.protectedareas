...
```{r line_plots-fcl, message = FALSE, warning = FALSE,fig.width=10, echo=FALSE, eval=TRUE}
# Library
## loss statistics
gfw_lossstats_2 <-
  read_csv(
...
# create unique names
gfw_lossstats_2$name_unique<-paste(gfw_lossstats_2$NAME,gfw_lossstats_2$wdpa_id,sep="_")

# create plot
fires_heatmap <-
  ggplot(gfw_lossstats_2,
         aes(x = year, y = as.factor(name_unique), fill = value)) +
  geom_tile() +
...
# create unique names
fires_long$name_unique<-paste(fires_long$NAME,fires_long$wdpa_id,sep="_")

# create plot
fires_heatmap <-
  ggplot(fires_long,
         aes(x = year, y = as.factor(name_unique), fill = active_fire_counts)) +
  geom_tile() +
  scale_fill_distiller(palette = "RdYlGn",na.value = 'salmon') +
  theme_classic()
