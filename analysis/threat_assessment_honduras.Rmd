...
wdpa_kfw <- read_sf("data/honduras_mangroves.gpkg")
wdpa_kfw <- st_make_valid(wdpa_kfw)
wdpa_kfw <- st_cast(wdpa_kfw, "POLYGON")
...
hnd_kfw_dissolved <- st_cast(hnd_kfw_dissolved, "POLYGON")
...
```{r map_forest_magrove, message = FALSE, warning = FALSE, echo = FALSE}
plot_one_pa <- function(dat, pa_name) {
  dat %<>%
    filter(dissolved == FALSE, min_cover == 10, Name == pa_name) %>%
    select(-X, -dissolved, -min_cover) %>%
...
mangrove_forest_combined <- merge(mangrovecover_areas, treecover_areas, all.x = TRUE, all.y = TRUE)
