...
# calculate disbursements per year
finance_data <- kfw_finance_aggregated %>% 
  mutate(projdur = last_year - first_year +1,
         disb_yr = total_disbursed/projdur) %>% 
  left_join(.,wdpa_bmz,
            by=c('bmz_no')) %>% 
  left_join(.,wdpa_info,
            by=c('wdpa_pid')) %>% 
  select(wdpa_pid, bmz_no, projdur, disb_yr, AREA_KM2) %>% 
  mutate(disb_yr_km2 = disb_yr/AREA_KM2)
```
...
```{r accessibility groups - line plot, echo=F}
gfw_kfw_data %>%
  inner_join(.,wdpa_accessibility,
             by=c('wdpa_pid')) %>%
...
```{r accessibility groups 2 - line plot, echo=F}
...
```{r accessibility and country - line plot, echo=F}
...
gfw_kfw_data %>%
  inner_join(.,wdpa_info,
             by=c('wdpa_pid')) %>%
  ggplot() +
  geom_histogram(aes(x=AREA_KM2), binwidth = 1000)
...
```{r inspect disb_yr_km2}
finance_data %>% 
  ggplot() +
  geom_histogram(aes(x=disb_yr_km2))

finance_data %>% 
  filter(disb_yr_km2<100000) %>% 
  ggplot() +
  geom_histogram(aes(x=disb_yr_km2))
```


```{r disb cutoffs, echo=T}
cutoffs_disb <-
...
```{r}
gfw_kfw_data %>% 
  inner_join(.,finance_data,
             by=c("wdpa_pid","bmz_no")) %>% 
  mutate(disb_yr_km2_bin = cut(disb_yr_km2, cutoffs_disb)) %>%
  ggplot() +
  geom_line(aes(x = rel_year, y = area_pct_projstart, group=interaction(wdpa_pid, bmz_no),
                col=disb_yr_km2_bin,
                alpha=0.4))
...
## Relative forest cover: Yearly averages

```{r yearly averages}
gfw_kfw_data %>%
  group_by(rel_year) %>%
  summarise(avg_areapct = mean(area_pct_projstart)) %>%
  ggplot(aes(x=rel_year, y=avg_areapct)) +
  geom_line() +
  geom_point(aes(size=3)) + 
  theme(legend.position = "none")
```

Let's have a look at the number of observations for each relative year: 

```{r number obs rel_years}
gfw_kfw_data %>% 
  ggplot() +
  geom_histogram(aes(x=rel_year),
                 binwidth = 1)
```
...
gfw_kfw_data_break <- gfw_kfw_data %>%
  mutate(D = as.factor(ifelse(rel_year >=treatmentgap, 1,ifelse(rel_year<=0, 0,NA)))) %>%
...
ggsave("lfit_ALL_withPoints.pdf", 
       path="../plots",
       width = 12,
       height = 7)
```
...
```{r no points less pre-trend, echo=FALSE, eval=F}
#without points
gfw_kfw_data_break %>%
  filter(rel_year>=(-pretrend)) %>% 
  ggplot(aes(x = rel_year, y = area_pct_projstart, color = D)) + 
...
```{r no points | alternative, eval = F}
gfw_kfw_data_break %>%
  filter(rel_year >= (-pretrend),
         rel_year <= posttreat
         ) %>%
...
gfw_kfw_data_break %>%
  inner_join(.,wdpa_accessibility,
             by=c('wdpa_pid')) %>%
  mutate(accessibility_bins = cut(travel_time_to_nearby_cities_min, 
                                  cutoffs_travel_time, 
                                  include.lowest = TRUE
                                  )) %>%
...
ggsave("lfit_accessgroups_ALL.pdf", 
       path="../plots",
       width = 12,
       height = 7)
```
...
* Is the deterioration of the trend in the groups with longer travel times driven by trends in a selected country? 


```{r accessibility less pre-trend, echo=F, eval=F}

# Set parameters
label_d <- c("D=0, <2h", "D=1, <2h", "D=0, 2-6h", "D=1, 2-6h", "D=0, >6h", "D=1, >6h")
label_nd <- c("0--2h","2--6h",">6h")

# without points, with accessibility bins 
gfw_kfw_data_break %>%
  inner_join(.,wdpa_accessibility,
             by=c('wdpa_pid')) %>%
  mutate(accessibility_bins = cut(travel_time_to_nearby_cities_min, cutoffs_travel_time, include.lowest = TRUE)) %>%
  filter(!is.na(accessibility_bins),
         rel_year >= (-pretrend),
         rel_year <= posttreat
         ) %>%
  ...
  stat_poly_eq(formula=y ~ x,
               label.x = "right",
               eq.with.lhs = "italic(hat(y))~`=`~",
               aes(label = paste("bold(\"", label_d[stat(group)],
                                 ": \")*",
                                 stat(eq.label),
                                 sep = "")),
               parse = TRUE) +
  labs(title="",
       ...
       color="Accessibility groups",
       linetype="Treatment") +
  scale_color_hue(labels = label_nd) +
  scale_linetype_discrete(labels = c("Pre-treatment", "Post-treatment")) +
  theme(axis.title = element_text(size = 16, face=3),
        axis.text = element_text(size = 16)) +
  coord_cartesian(ylim=c(ylower,yupper))
...
```{r no points accessibility alt2 cutoffs, echo=F, eval=F}
# corresponding labels and cutoffs: 
label_d <- c("D=0, <2h", "D=1, <2h", "D=0, 2-4h", "D=1, 2-4h", "D=0, 4-6h", "D=1, 4-6h", "D=0, >6h", "D=1, >6h")
label_nd <- c("0--2h","2--4h","4--6h",">6h")
cutoffs <- cutoffs_travel_time_alt2
...
gfw_kfw_data_break %>%
  inner_join(.,wdpa_accessibility,
             by=c('wdpa_pid')) %>%
  mutate(accessibility_bins = cut(travel_time_to_nearby_cities_min, cutoffs, include.lowest = TRUE)) %>%
  filter(!is.na(accessibility_bins),
         rel_year >= (-pretrend),
         rel_year <= posttreat
         ) %>%
...
table(cut(inner_join(gfw_kfw_data_break, wdpa_accessibility, 
                     by=c("wdpa_pid"))$travel_time_to_nearby_cities_min,
          cutoffs_travel_time_alt2, 
          include.lowest = TRUE))
```
...
# corresponding labels and cutoffs: 
label_d <- c("D=0, <1h", "D=1, <1h", "D=0, 1-2h", "D=1, 1-2h", "D=0, 2-6h", "D=1, 2-6h", "D=0, >6h", "D=1, >6h")
label_nd <- c("0--1h","1--2h","2--6h",">6h")
cutoffs <- cutoffs_travel_time_alt3
...
gfw_kfw_data_break %>%
  ...
  ggplot(aes(x = rel_year, 
             y = area_pct_projstart, 
             color = accessibility_bins, 
             linetype = D)) + 
  # geom_point(position = "jitter", alpha = 0.3) +
  geom_smooth(method = "lm") +
  stat_poly_eq(formula=y ~ x,
  ...
  theme(axis.title = element_text(size = 16, face=3),
        axis.text = element_text(size = 16)) +
  coord_cartesian(ylim=c(ylower,yupper))# ggplotly()
# ggplotly()
```

```{r accessibility bins 3, echo=F}
table(cut(inner_join(gfw_kfw_data_break, wdpa_accessibility, by=c("wdpa_pid"))$travel_time_to_nearby_cities_min,cutoffs_travel_time_alt3, include.lowest = TRUE))
```
...
ggsave("lfit_accessgroups_BRAZIL.pdf", 
       path="../plots",
       width = 12,
       height = 7)
```
...
### Grouped by accessibility bins, all but selected country

```{r no points accessibility all but selected country, echo=F}
# corresponding labels and cutoffs: 
label_d <- c("D=0, <2h", "D=1, <2h", "D=0, 2-6h", "D=1, 2-6h", "D=0, >6h", "D=1, >6h")
label_nd <- c("0--2h","2--6h",">6h")
cutoffs <- cutoffs_travel_time
...
gfw_kfw_data_break %>%
  ...
  stat_poly_eq(formula=y ~ x,
               ...
               aes(label = paste("bold(\"", 
                                 ...
                                 sep = "")),
               parse = TRUE) +
  labs(title=paste("All countries, excluding:", thiscountry),
       x="\n Year, relative to project start",
       y="Forest cover, relative to project start \n",
       color="Accessibility groups",
       linetype="Treatment") +
...
ggsave("lfit_accessgroups_nonBRAZIL.pdf", 
       path="../plots",
       width = 12,
       height = 7)
```
...
label_d <- c("D=0, <115 km2", 
             ...
             "D=0, >2743 km2", 
             "D=1, >2743 km2")
label_nd <- c(paste("<",round(cutoffs_size_2[2],2)," km2",sep=""),
              paste(round(cutoffs_size_2[2],2),"--",round(cutoffs_size_2[3],2)," km2",sep=""),
              paste(round(cutoffs_size_2[3],2),"--",round(cutoffs_size_2[4],2)," km2",sep=""),
              paste(">",round(cutoffs_size_2[4],2)," km2",sep=""))
# label_nd <- c("<115km2","115-682km2","682-2743km2",">2743km2") 
cutoffs <- cutoffs_size_2 #Quartile cutoffs

gfw_kfw_data_break %>%
  inner_join(.,wdpa_info,
             by=c('wdpa_pid')
             ) %>%
  mutate(size_bin = cut(AREA_KM2, 
                        cutoffs, 
                        include.lowest = TRUE
                        )) %>%
  filter(rel_year >= (-pretrend),
