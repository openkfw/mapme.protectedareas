---
title: "Panel creation tn - Project level"
...
projectdata_database <-
  ...
  select("bmz_nummer", "first_year", "last_year", "total_disbursed", "Titel", "Finanzierungsform", "Mittelart" ) %>% 
  rename(first_year_bmz = first_year,
         last_year_bmz = last_year)
...
```{r}
# Load action types
actiontypes <- read_rds("/datadrive/yota/test/actiontypes_bybmzpa.RDS") %>% 
  mutate(id = as.numeric(if_else(id == "N.N.", NA_character_, id))) %>% 
  rename(bmzno = bmz, 
         WDPAID = id) %>% 
  pivot_longer(c(-bmzno, -WDPAID)) %>% 
  mutate(value = recode(value, 
                        `2` = 1,
                        `1` = 0,
                        `0` = 0)) %>% # Currently, 1 = "strong focus", 0 = "weak or no focus"
...
cell.agg<- bmz_wdpa.df %>% 
  dplyr::group_by(.assetid, year) %>% 
  dplyr::summarize(cell_disb=sum(disbursement_5sqkmcell))

```

## Panelize PSM data

```{r Panelize PSM data, eval = FALSE}
# BMZ with matching frames:
T_year <- list.files("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/PSM/tn_projects")%>% 
  grep("psm_tn_matched_data_*", ., value = TRUE)

for (i in T_year) {

  bmz <- gsub("psm_tn_matched_data_", "", i) %>% 
    gsub(".rds", "", .)
  ...
  # Load matched data
    m_data <-
      read_rds(paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/PSM/tn_projects/psm_tn_matched_data_", bmz, ".rds")) %>%
      rename("m_treecover" = "treecover",
             "m_loss" = "loss",
             "m_loss_tn" = "loss_tn",
             # "m_loss_tn" = "loss_tn",
             "m_emissions" = "emissions") %>% 
      select(".assetid", "WDPAID", "treatment", "m_treecover", "m_emissions", "m_loss", "m_loss_tn",
 "traveltime_5k_110mio", "traveltime_20k_110mio", "terrain_ruggedness_index_mean", "elevation_mean", "soil_5_15cm_clay", "NAME_0", "first_year", "second_year", "third_year", "distance", "weights")
    
# Load financing data
    m_data <- m_data %>% 
      mutate(bmz_nummer = as.numeric(bmz)) %>% 
      left_join(., projectdata_database,
                             by=c("bmz_nummer"))
    ...
    merged_data <- 
      ...
      left_join(., wdpa_type,
                             by=c("WDPAID")) %>% 
      mutate(year_standard = as.numeric(year) - first_year) %>% 
    ...
    write_csv(merged_data,
              paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/PSM/tn_projects/psm_tn_matched_panel_", bmz, ".csv"))
...
for (i in T_year) {
    ...
    bmz <- gsub("cem_tn_matched_data_", "", i) %>% 
    gsub(".rds", "", .)
    print(bmz)
  
    # Load matched data
    m_data <- 
        read_rds(paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/tn_projects/cem_tn_matched_data_", bmz, ".rds")) %>%
      rename("m_treecover" = "treecover",
             "m_loss" = "loss",
             "m_loss_tn" = "loss_tn",
             # "m_loss_tn" = "loss_tn",
             "m_emissions" = "emissions") %>% 
      select(".assetid", "WDPAID", "treatment", "m_treecover", "m_emissions", "m_loss", "m_loss_tn",
 "traveltime_5k_110mio", "traveltime_20k_110mio", "terrain_ruggedness_index_mean", "elevation_mean", "soil_5_15cm_clay", "NAME_0", "first_year", "second_year", "third_year", "weights")
    ...
    # Load relevant actiontypes
    actiontypes_filtered <- actiontypes %>% 
      filter(bmzno == bmz)
    ...
    merged_data <- 
      ...
      filter(! is.na(year)) %>% # new line
      distinct(.assetid, year, .keep_all = TRUE) %>%  # adjust this as soon as we have a better solution # new line  
      left_join(., actiontypes_filtered, by = c("WDPAID"))
    
    ## Export data
    write_csv(merged_data, 
              paste0("/datadrive/datalake/mapme.protectedareas/output/tabular/regression_input/CEM/tn_projects/cem_tn_matched_panel_", bmz, ".csv"))
